...
if __name__ == "__main__":
    ...
    popularities = joint_dst.sample_popularities(no_objects * 3)
    popularities = np.append(popularities, [1] * (2*no_objects))    

    joint_dst = pop_sz_dst("results/" + w_dir + "/joint_dst.txt")
    sizes = []
    for i in range(len(popularities)):
        p = popularities[i]
        sizes.append(int(joint_dst.sample(p)/1000) + 1)
    ...
    fd_sample = pop_sz_dst("results/" + w_dir + "/fd_bytes.txt", "pop") 
    sampled_fds = []
    ...
    while curr != None and i <= 1.4 * t_len:
        ...
        if end_object == False:
            ...
            local_uniq_bytes = curr.findUniqBytes(n, debug)
            
            debug.write(str(local_uniq_bytes) + " " + str(sd) + " " + str(root.s) + " " + str(descrepency) + "\n")

            object_landed_on.append(o_id)
            if sd > 350000:
                descs1[descrepency] += 1
                land_position1[sizes[o_id]].append(land)
            else :
                descs2[descrepency] += 1
                land_position2[sizes[o_id]].append(land)
