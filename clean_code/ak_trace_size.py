...
if __name__ == "__main__":
    ...
    land_position = defaultdict(list)
    object_landed_on = []
    ...
    while curr != None and i <= 1.4 * t_len:
        ...
        if end_object == False:
            descrepency, land, o_id = root.insertAt(sd, n, 0, curr.id)                

            land_position[sizes[o_id]].append(land)
    ...
    with open("results/" + w_dir + "/descrepency.json", 'w') as outfile:
        json.dump(descs, outfile)

    ## f) Distribution of where exactly the object landed (in buckets of 0.001)
    with open("results/" + w_dir + "/land_position.json", 'w') as outfile:
        json.dump(land_position, outfile)
