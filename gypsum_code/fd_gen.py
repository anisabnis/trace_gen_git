...
sub_dir = sys.argv[1]
data_path = "/mnt/nfs/scratch1/asabnis/data/binary/small/"
...
def st_gen(trace_list, sd_gran, iat_gran, st):
	...
	for t in st.keys():
		for s in st[t].keys():
			#ohr, bhr = st[t][s]
			st[t][s] = (float(st[t][s][0])/count, float(st[t][s][1])/bytes)
	# Delete iat 0 -- CHECK?
	del(st[-1])	
	return (count, bytes)
...
if __name__ == '__main__':
	...
	#trace_in = sys.argv[1]
        trace_in = data_path + "/" + sub_dir + "/subtrace.txt"
	...
	# Print st
	st_f = data_path + "/" + sub_dir + '/st_out'
	f_print_out_st(st_f, st, rb_total[0])
	# Print sd
	sd_f = data_path + "/" + sub_dir + '/sd_out'
