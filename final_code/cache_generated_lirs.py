...
lirs_c = LIRSCache(cache_size)
objects = set()

if BR == "r":
    f = open("results/" + str(tc) + "/simulations/generated_lirs_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
else:
    f = open("results/" + str(tc) + "/simulations/byte_generated_lirs_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
...
for r in reqs:
    ...
    if total_sz <= cache_size:
        ...
        lirs_c.initialize(initial_objs, initial_sizes, initial_tms)
        initialized = True
    else:
        sd = lirs_c.insert(r, obj_sz, 0)
        if sd != -1:
            if i > ignore:
                obj_hits += 1
                byte_hits += obj_sz
