...
if __name__ == "__main__":
    ...
    f = open("results/" + w_dir + "/footprint_desc_unit.txt", "r")
    l = f.readline().strip().split(" ")
    one_hit_pr = float(l[-1])/float(l[0])
    f.close()

    fd_sample = pop_opp2("results/" + w_dir + "/footprint_desc_unit.txt", 0, TB)
    ...
    unit_sizes = [1] * (total_objects + 1)
    
    ## create a tree structure using the initial tree
    trace_list, ss = gen_leaves(trace, unit_sizes)
...
#            while root.s > 10*TB:
    ...
    f = open("results/" + w_dir + "/sampled_sizes_lrusm_unit.txt", "w")
    f.write(",".join([str(x) for x in sizes]))
    f.close()
        
    # ## Write stats to disk
    f = open("results/" + w_dir + "/sampled_fds_lrusm_unit.txt", "w")
    for i in range(len(sampled_fds)):
        f.write(str(sampled_fds[i]) + ",")
    f.close()

    # ## Write the trace to dist
    f = open("results/" + w_dir + "/out_trace_lrusm_unit.txt", "w")
