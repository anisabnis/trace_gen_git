...
slru_c = SLRUCache(4, cache_size/4)
objects = set()

if BR == "r":
    f = open("results/" + str(tc) + "/simulations/generated_s4lru_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
else:
    f = open("results/" + str(tc) + "/simulations/byte_generated_s4lru_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
