...
lfu_c = LFUCache(cache_size)
objects = set()

if BR == "r":
    f = open("results/" + str(tc) + "/simulations/generated_lfu_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
else:
    f = open("results/" + str(tc) + "/simulations/byte_generated_lfu_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
