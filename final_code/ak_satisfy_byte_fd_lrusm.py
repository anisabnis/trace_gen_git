...
if __name__ == "__main__":
    ...
    f = open("results/" + w_dir + "/byte_footprint_desc_all.txt", "r")
    l = f.readline().strip().split(" ")
    one_hit_pr = float(l[-1])/float(l[1])
    f.close()

    fd_sample = pop_opp2("results/" + w_dir + "/byte_footprint_desc_all.txt", 0, 1000*TB)
    ...
    f = open("results/" + w_dir + "/byte_sampled_sizes_lrusm.txt", "w")
    f.write(",".join([str(x) for x in sizes]))
    f.close()
        
    # ## Write stats to disk
    f = open("results/" + w_dir + "/byte_sampled_fds_lrusm.txt", "w")
    for i in range(len(sampled_fds)):
        f.write(str(sampled_fds[i]) + ",")
    f.close()

    # ## Write the trace to dist
    f = open("results/" + w_dir + "/byte_out_trace_lrusm.txt", "w")
