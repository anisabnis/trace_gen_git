...
markers_c = MarkersCache(cache_size)
objects = set()

if BR == "r":
    f = open("results/" + str(tc) + "/simulations/generated_markers_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
else:
    f = open("results/" + str(tc) + "/simulations/byte_generated_markers_" +  str(cache_size) + "_" + str(typ) + ".txt", "w")
...
for r in reqs:
    ...
    ## Make reqest to SLRU
    if markers_c.get(r) == -1:
        markers_c.put(r, obj_sz)
    else:
        if i > ignore:
            obj_hits += 1
            byte_hits += obj_sz
