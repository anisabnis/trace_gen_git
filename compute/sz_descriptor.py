...
f = open("/mnt/nfs/scratch1/asabnis/trace_gen/" + t_file + "/size_sd_" + str(obj_mod) + ".txt", "w")
...
for obj in sd_distances:
    sz = obj_sizes[obj]
    sd_distances_[sz].extend(sd_distances[obj])

sz_keys = list(sd_distances_.keys())
sz_keys.sort()
for sz in sz_keys:
    keys, vals = convert_to_dict(sd_distances_[sz], total_reqs)
    for i in range(len(keys)):
        f.write(str(sz) + " " + str(keys[i]) + " " + str(vals[i]) + "\n")
f.close()
...
f = open("/mnt/nfs/scratch1/asabnis/trace_gen/" + t_file + "/joint_dst_" + str(obj_mod) + "_sz.txt", "w")
for p in pop_sz:
    f.write(str(p) + "\n")
